...
public class Destruction : MonoBehaviour
{
    ...
    public Vector2 durabilityRange = new Vector2(1f, 3f);
    //Private vars
    ...
    void Start()
    {
        durability = Random.Range((int)durabilityRange.x, (int)durabilityRange.y);
        //Get the rigidbodies
    ...
    private void OnCollisionEnter(Collision collision)
    {
        ...
        {
            if (stampedCount == durability)
            {
                Break();
                foreach (var i in GetComponentsInChildren<Ingredient>())
                {
                    i.SetSmashableDelay(0.5f);
                }
                this.enabled = false;
            }
            else
